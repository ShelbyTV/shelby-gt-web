libs.shelbyGT.DynamicRouter = Backbone.Router.extend({
  ...
  displayRoll: function(roll, title, params, options, topLevelViewsOptions) {
    ...
    if (options.updateRollTitle && rollId == shelby.models.user.get('personal_roll_id')) {
      ...
      this.navigate(shelby.models.user.get('nickname'), {
        trigger: false,
        replace: true
      });
    }
  ...
  displayEntryInChannel: function(channel, entryId, params) {
    ...
    if (_(shelby.config.channels).has(channel)) {
      ...
      // if the requested channel doesn't exist, just go to the first channel
      this.navigate('channels/community/' + entryId, {
        trigger: true,
        replace: true
      });
    }
  ...
  displayRollFromFrame: function(frameId, params) {
    var self = this;
    var frameModel = new libs.shelbyGT.FrameModel({
      id: frameId
    });
    frameModel.fetch({
      success: function(frameModel, response) {
        if (frameModel.has('roll_id')) {
          self.navigate('roll/' + frameModel.get('roll_id'), {
            trigger: true,
            replace: true
          });
        } else {
  ...
  displayUserPersonalRoll: function(userId, params) {
    var self = this;
    var roll = new libs.shelbyGT.UserPersonalRollModel({
      creator_id: userId
    });
  ...
  displayDashboard: function(params, options) {
    ...
    shelby.trackEx({
      ...
      gaValue: 1,
      kmqProperties: {
        user: shelby.models.user.get('nickname')
      }
    });
    ...
    options = _.chain({}).extend(options).defaults({
      onDashboardFetch: function(dashboardModel, response) {
        if (response.result.length) {
          ...
          if (shelby.models.guide.get('displayState') == libs.shelbyGT.DisplayState.dashboard && !shelby.models.guide.get('hasShownDynamicRec')) {
            // we're not going to do anything until we have some real dashboard entries
            if (shelby.models.dashboard.get('dashboard_entries').length && shelby.promises.dynamicRecommendationsFetch) {
                ...
                if (shelby.models.guide.get('displayState') == libs.shelbyGT.DisplayState.dashboard) {
                  libs.utils.recommendationPlacer.placeRecs(
                    ...
                    shelby.collections.dashboardFrameGroupsCollection, {
                      destinationCollectionFilter: function(dbe) {
                        return !dbe.isNotificationEntry();
                      }
                    }
                  );
  ...
  displaySaves: function() {
    ...
    if (watchLaterRoll) {
      ...
      shelby.alert({
        message: "<p>Could not roll view to your Queue</p>"
      });
      this.navigate('', {
        trigger: true,
        replace: true
  ...
  displayUserPreferences: function(section) {
    ...
    shelby.models.userPreferencesView.set({
      section: section
    });
    shelby.models.guide.set({
  ...
  displayUserProfile: function(userName, params) {
    if (userName == shelby.models.user.get('nickname')) {
      this.displayRoll(shelby.models.user.get('personal_roll_id'), null, params, {
        updateRollTitle: false
      });
    } else {
  ...
  _activateFrameInRollById: function(rollModel, frameId, showOverlayType) {
    ...
    if (frame && (!Browser.isMobile() || frame.get('video').canPlayMobile())) {
      ...
      shelby.alert({
        message: "Sorry, the video you were looking for doesn't exist in this roll."
      });
      this.navigateToRoll(rollModel, {
        trigger: true,
        replace: true
      });
    }
  ...
  _activateEntryInDashboardById: function(dashboardModel, entryId) {
    ...
    if (!frameActivated) {
      ...
      if (dashboardModel.has('channel')) {
        ...
        shelby.alert({
          message: "<p>Sorry, the video you were looking for doesn't exist in this channel.</p>"
        });
        this.navigate("channels/" + dashboardModel.get('channel'), {
          trigger: true,
          replace: true
        });
      } else {
        ...
        shelby.alert({
          message: "<p>Sorry, the video you were looking for doesn't exist.</p>"
        });
        this.navigate("", {
          trigger: true,
          replace: true
  ...
  _setupTopLevelViews: function(options) {
    ...
    shelby.views.guideOverlayManager = shelby.views.guideOverlayManager ||
      new libs.shelbyGT.GuideOverlayManagerView({
        model: shelby.models.guideOverlay,
        el: '.js-action-layout'
      });
    shelby.views.guideSpinner = shelby.views.guideSpinner ||
      new libs.shelbyGT.SpinnerView({
        el: '#guide',
        size: 'large-light'
      });
  ...
  _setupRollView: function(roll, title, options, topLevelViewsOptions) {
    ...
    if (typeof(roll) === 'string') {
      ...
      rollModel = followedRoll || new libs.shelbyGT.RollModel({
        id: roll
      });
    } else {
    ...
    var oneTimeSpinnerState = new libs.shelbyGT.SpinnerStateModel();
    shelby.views.guideSpinner.setModel(oneTimeSpinnerState);
    $.when(rollModel.fetch(fetchOptions)).always(function() {
      oneTimeSpinnerState.set('show', false);
    });
  },

libs.shelbyGT.UserProfileInfoView = Support.CompositeView.extend({
  ...
  _followOrUnfollowRoll : function(e) {
    ...
    var isUnfollow = $thisButton.toggleClass('js-roll-unfollow button_green button_gray').hasClass('js-roll-unfollow');
    ...
    $thisButton.text(isUnfollow ? 'Following' : 'Follow').addClass('js-busy');
  ...
  _updateFollowButton : function() {
     ...
     if (currentRoll) {
       if (libs.shelbyGT.viewHelpers.roll.isFaux(currentRoll)){
         ...
         this.$('.js-follow-button').toggleClass('js-roll-unfollow button_gray', userFollowingRoll)

(function(){
  ...
  var svg = d3.select(".js-force-graph").append("svg")
    ...
    var people = svg.selectAll(".people").data(graph.nodes).enter()
      ...
      .html(function(d){
        if (d.group=='video') {
          ...
          return '<image src="' + d.picture + '" class="' + d.group + '"/>';
        }
      })
      .attr("class", function(d){
        if (d.group == 'social') {
          return "social__wrapper social--" + d.name;
        } else {
          return d.group + "__wrapper";
        }
       }).call(node_drag);
    ...
    force.on("tick", function() {
      ...
      .attr("x", function(d) {
        if (d.group!="profile") {
          if (Math.abs(d.x - d.originX) < 1) {
            d.x = d.originX;
          } else {
            d.x += (graph.nodes[0].x - d.x - d.originX) * speed;
          }
          return d.x;
        } else {
          return (d.x-d.radius);
      ...
      .attr("y", function(d) {
        if (d.group!="profile") {
          if (Math.abs(d.y - d.originY) < 1) {
            d.y = d.originY;
          } else {
            d.y += (graph.nodes[0].y - d.y - d.originY) * speed;
          }
        return d.y;
      }
        return (d.y - d.radius);

...
Backbone.Router.decodeFragment = true;
    ...
    if (value.indexOf(splitChar) >= 0) {
      ...
      for (var i=values.length-1; i>=0; i--) {
        if (!values[i]) {
          ...
          values[i] = Backbone.Router.decodeFragment ? decodeQueryComponent(values[i]) : values[i];
        }
      }
      return values;
    }
    if (!currentValue) {
      return Backbone.Router.decodeFragment ? decodeQueryComponent(value) : value;
    } else if (_.isArray(currentValue)) {
      currentValue.push(Backbone.Router.decodeFragment ? decodeQueryComponent(value) : value);
      return currentValue;
    } else {
      return [currentValue, Backbone.Router.decodeFragment ? decodeQueryComponent(value) : value];
    }
  },
...
function decodeQueryComponent(encodedComponent) {
  // handle spaces that are encoded as + in the query, which decodeURICompoent doesn't do
  return decodeURIComponent((encodedComponent+'').replace(/\+/g, '%20'));
}
